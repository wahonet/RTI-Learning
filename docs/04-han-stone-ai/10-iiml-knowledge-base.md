...
新增参考：`papers/reading/06-ai-annotation-line-drawing/35-ICON_Ontology_Iconology/36-ICON_Ontology_Iconology_note.md`

ICON Ontology提示本项目应把图像解释分层保存，而不是把所有标签混在一个字段里。建议IIML第一版增加或预留：
- `interpretationLevel`：前图像、图像志、图像学。
- `evidenceResource`：支撑判断的原图、RTI增强图、拓片、线图、论文或馆藏记录。
- `agent`：人工研究者、标注员或AI模型。
- `reviewStatus`：候选、已审核、已拒绝。

这样可以把YOLO/SAM生成的几何候选和真正的汉画像石图像学解释分开。
...
本仓库配套的机器校验草案见 `docs/04-han-stone-ai/iiml-jsonld.schema.json`。
...
新增参考：`papers/reading/06-ai-annotation-line-drawing/24-Han_Portrait_Stone_YOLOv5/23-Han_Portrait_Stone_YOLOv5_note.md`

YOLO适合作为汉画像石图像元素的粗定位工具。第一版建议检测少量高价值类别，例如伏羲女娲、乐舞人物、车马、神兽、建筑构件、铭文/榜题。YOLO输出进入IIML时应保持候选状态：

```json
{
  "id": "urn:uuid:detection-001",
  "resourceId": "urn:uuid:resource-rti-enhanced-001",
  "target": {
    "type": "BBox",
    "xywh": [120, 340, 280, 420]
  },
  "semantics": {
    "label": "伏羲女娲",
...
YOLO bbox不应直接作为最终研究标注。推荐后续用SAM或人工多边形细化，再补充图像学关系。
